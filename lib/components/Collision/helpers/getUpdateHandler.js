...
export default (entity, block, { rigid, collision }) => {
    ...
    return () => { // update handler with collison resolution logic
        ...
        collision.test(block => {
            ...
            collision.onHit(block, velX, velY, !(entity.pos.x === entity.prevPosX)) // last argument is a flag indicating whether the ball (moved) collided from non colliding position
            xResolved = true
        }, true)
        ...
        collision.test(block => {
            ...
            collision.onHit(block, velX, velY, !(entity.pos.y === entity.prevPosY))
        }, false)
    }
}
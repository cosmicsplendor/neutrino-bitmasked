...
export default class Movement {
    ...
    static updateOffEdge(entity, dt) {
        ...
        const dPos = (entity.velX * tanX + 50 * tanY) * dt // dPos projection along tangent (this is the amount by which to move along circumference)
        ...
        const dx = entBounds.radius * (1 + Math.cos(a1)) + hitCirc.x // x-offset on circle which should now snap to the corner
        const dy = entBounds.radius * (1 + Math.sin(a1)) + hitCirc.y // y-offset on circle which should now snap to the corner

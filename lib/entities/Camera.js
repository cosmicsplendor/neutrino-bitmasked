...
class Camera extends Node {
    ...
    setYTracking(yBottom, height) {
        this.yTracker = {
            yc: yBottom - height / 2, yt: yBottom-height, yb: yBottom, height
        }
    }
    getTargetY() {
        const subY = this.subject.pos.y
        const {yc, yt, yb} = this.yTracker
        const targetY = yc - this.bounds.height * 0.5
        if (subY > yb) {
            return targetY + (subY - yb)
        }
        if (subY < yt) {
            return targetY - (yt - subY)
        }
        return targetY
    }
    intersects(node) {
    ...
    focusInst() { // focus instantly
        ...
        this.pos.y = this.pF * (-clamp(0, this.world.height - this.bounds.height, this.getTargetY()))
    }
    focus(dt) {
        if (!this.subject) { return }
        const destX = this.pF * (-clamp(0, this.world.width - this.bounds.width, this.subject.pos.x + this.offset.x - this.bounds.width / 2))
        const destY = this.pF * (-clamp(0, this.world.height - this.bounds.height, this.getTargetY()))

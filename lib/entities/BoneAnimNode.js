...
class BoneAnimNode extends Node {
    ...
    syncRecursively(bone = this.rootBone, globalAnchorPos = { x: 0, y: 0 }, globalAngularPos = 0) {
        ...
        bone.childBones.forEach(childBone => {
            this.syncRecursively(childBone, globalAnchorPos, bone.rotation)
        })
    }
    ...
    locatePointOnBone({ boneName, normX, normY }) {
        ...
        return {
            x: transformOriginX + pointVecLen * Math.cos(pointAngle + bone.rotation * Math.PI / 180),
            y: transformOriginY + pointVecLen * Math.sin(pointAngle + bone.rotation * Math.PI / 180)
        }
    }

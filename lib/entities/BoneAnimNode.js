...
const findAnchorBone = (bone, prevState) => {
    ...
    for (const childBone of bone.childBones) {
        const anchorBone = findAnchorBone(childBone)
        if (anchorBone) return anchorBone
    }

    return null
}
const getBoneEnd = (bone) => {
    const netAngle = bone.rotation
    return {
        x: bone.pos.x + bone.anchor.x + bone.length * Math.cos(netAngle),
        y: bone.pos.y + bone.length * Math.sin(netAngle)
    }
}
...
class BoneAnimNode extends Node {
    ...
    update(dt) {
        ...
        if (this.elapsedTime >= this.maxDuration) {
         ...
         const anchorBone = findAnchorBone(skeletalSceneGraph, this.prevState)
         if (anchorBone) {
             sceneGraph.pos.x = -(anchorBone.pos.x + anchorBone.anchor.x + anchorBone.length * Math.cos(netAngle))
             sceneGraph.pos.y = -(anchorBone.pos.y + anchorBone.anchor.y + anchorBone.length * Math.sine(netAngle))
         }
    }
}

...
class Particle extends TexRegion {
    constructor({ imgId, metaId, frame, pos, lifetime, velX=0, velY=0, accX, accY, alpha, alphaDecayFn, scaleDecayFn, rotVel, loop }) {
    ...
    update(dt) {
        ...
        if (this.alphaDecayFn) {
            this.alpha = Math.max(0.01, 1 - easingFns[this.alphaDecayFn](n))
        }
        if (this.scaleDecayFn) {
...
class ParticleEmitter extends Node {
    ...
    constructor({ size, blendMode, loop = false, randomDistribution = true, metaId, imgId, params, ...nodeProps }) {
        ...
        for (let i = 0; i < size; i++) {
            ...
            console.log(easingFns[param.alphaDecayFn] ? param.alphaDecayFn: null,)
            const deserializedParam = {
                ...
                alphaDecayFn: easingFns[param.alphaDecayFn] ? param.alphaDecayFn: null,
                scaleDecayFn: easingFns[param.scaleDecayFn] ? param.scaleDecayFn: null,
                loop
            }

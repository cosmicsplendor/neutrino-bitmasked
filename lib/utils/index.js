...
export class BiomeGen {
    ...
    generate() {
        ...
        if (!nextNode) {
            this.transition();
            stateMachine.excessX = 0
            return stateMachine.generate();
        }

        const { name, gap, dims, flip } = nextNode;

        const numBases = Math.ceil((dims.width + gap - stateMachine.excessX) / stateMachine.baseWidth);
        const tiles = [];
        ...
        const extension = numBases * stateMachine.baseWidth
...
export class ObjGraph {
    ...
    _formatNextNode(nodeName, gap = 0) {
        ...
        const randGap = Array.isArray(gap) ? 
                rand(gap[1], gap[0]):
                gap <= 0 ? gap: (gap  + rand(gap) )* 0.75
        return {
            name: name,
            flip: flipped,
            gap: randGap,

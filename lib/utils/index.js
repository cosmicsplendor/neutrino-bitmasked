import { pickOne, rand, sqDist } from "./math"
...
export const fadeSound = (sound, cutoff, entity) => {
    const dist = sqDist(entity.pos, entity.player.pos)
    if (dist < cutoff) {
        if (!sound.playing) {
            sound.play()
        }
        sound.volume = 1 - dist/cutoff
    } else {
        sound.pause()
    }
}
...
export class BGenMachine {
    ...
    mapToEntities(tiles) {
        for (let i = 0; i < tiles.length; i++) {
            ...
            const entity = new Entity({ frame: tile.t, pos: { x: tile.x, y: config.viewport.height  + tile.y, }, scale: tile.flip ? { x: -1, y: 1 }: undefined })
            if (tile.flip) {
                entity.pivot = { x: tile.dims.width, y: 0}
            }
            entity.y0 = tile.y
            tiles[i] = entity
        }
...
export class BiomeGen {
    ...
    generate() {
        ...
        if (!nextNode) {
            ...
            if (curH < lastH) {
                ...
                for (let row = 0; row < dh; row++) {
                    ...
                    const dims = this.objGraph.dimsMap[frame]
                    const  { height } = dims
                    results.push({ t: frame, y: - (lastH + row) * baseHeight - height, x: curX + row * baseWidth + baseWidth, flip: true, dims })
                    for (let col = row + 1; col < dh + 1; col++) {
        ...
        tiles.push({
            t: name,
            x: gap + stateMachine.curX - stateMachine.excessX,

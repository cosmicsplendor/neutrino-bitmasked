...
export class BiomeGen {
    ...
    generate() {
        ...
        if (!nextNode) {
            this.transition();
            return this.stateMachine.generate();
        }
        ...
        const numBases = Math.ceil((dims.w + gap) / baseWidth);
        const tiles = [];
        ...
        for (let i = 0; i < numBases; i++) {
            for (let j = 0; j < this.height; j++) {
                tiles.push({
                    t: "base",
                    x: this.stateMachine.curX + i * baseWidth,
                    y: -(j + 1) * baseHeight,
                });
            }
        }
        ...
        tiles.push({
            t: name,
            x: this.stateMachine.curX,
            y: -dims.h - this.height * baseHeight,
        });

        this.stateMachine.curX += numBases * baseWidth;
